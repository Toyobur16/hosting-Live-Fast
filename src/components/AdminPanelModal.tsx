...
export const AdminPanelModal: React.FC<AdminPanelModalProps> = ({
  ...
  const handleTestBinanceConnection = async () => {
    ...
    const token = localStorage.getItem('bot_auth_token');
    try {
      const res = await fetch('/api/admin/binance-pay/test-connection', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
        ...
        body: JSON.stringify({
          apiKey: paymentSettings.binancePayApiKey,
          secretKey: paymentSettings.binancePaySecretKey
        })
      });
      const data = await res.json();
      if (!res.ok || !data.success) {
        setBinanceTestResult({ success: false, message: data.error || 'সংযোগ পরীক্ষা ব্যর্থ হয়েছে' });
      } else {
        setBinanceTestResult({ success: true, message: data.message || 'সংযোগ সফল হয়েছে!' });
      }
    } catch (err: any) {
      setBinanceTestResult({ success: false, message: err.message || 'Network error' });
    } finally {
      ...
      <div className="bg-[#0b1120] border-0 sm:border border-[#1e2e48] shadow-2xl rounded-none sm:rounded-3xl max-w-6xl w-full p-2.5 sm:p-5 text-white relative h-full sm:h-[95vh] max-h-[100dvh] sm:max-h-[95vh] flex flex-col overflow-hidden">
        ...
        {activeTab === 'payments' && (
          <form onSubmit={handleSavePaymentSettings} className="space-y-4 pr-1">
            {/* ⚡ Binance Pay Official Automated Gateway Section */}
            <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-br from-amber-500/10 via-[#0d1424] to-[#070b14] border border-amber-500/30 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-amber-500/20">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="text-sm font-black text-white">⚡ Binance Pay অটোমেটিক ইনস্ট্যান্ট গেটওয়ে</h4>
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-400/20 text-amber-300 font-bold">
                        Official Merchant API
                      </span>
                    </div>
                    <p className="text-[11px] text-slate-400 mt-0.5">
                      ইউজার সরাসরি বাইন্যান্সে পেমেন্ট কনফার্ম করবে এবং সাথে সাথে একাউন্টে ব্যালেন্স যুক্ত হবে।
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`text-[10px] px-2.5 py-1 rounded-full font-bold flex items-center gap-1 ${
                      paymentSettings.hasBinanceCredentials ||
                      (paymentSettings.binancePayApiKey && paymentSettings.binancePaySecretKey)
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer text-xs text-white font-bold bg-[#0f172a] px-3 py-1.5 rounded-xl border border-slate-700">
                    <input
                      type="checkbox"
                      checked={paymentSettings.binancePayApiEnabled !== false}
                      onChange={(e) =>
                        setPaymentSettings({ ...paymentSettings, binancePayApiEnabled: e.target.checked })
                      }
                      className="rounded text-amber-400 focus:ring-0"
                    />
                    <span>গেটওয়ে চালু</span>
                  </label>
                </div>
              </div>

              {/* Webhook notification URL with 1-click copy */}
              <div className="p-3 rounded-xl bg-[#090d18] border border-amber-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                <div>
                  <span className="text-[11px] font-bold text-amber-300 block">
                    আপনার Binance Pay Webhook Callback URL:
                  </span>
                  <span className="font-mono text-[11px] text-slate-300 select-all break-all">
                    {typeof window !== 'undefined' ? `${window.location.origin}/api/binance-pay/webhook` : '/api/binance-pay/webhook'}
                  </span>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    if (typeof window !== 'undefined') {
                      navigator.clipboard.writeText(`${window.location.origin}/api/binance-pay/webhook`);
                      setCopiedWebhook(true);
                      setTimeout(() => setCopiedWebhook(false), 2000);
                    }
                  }}
                  className="px-3 py-1.5 rounded-lg bg-amber-400/20 hover:bg-amber-400/30 text-amber-300 font-bold text-xs shrink-0 cursor-pointer transition-colors"
                >
                  {copiedWebhook ? '✓ কপি হয়েছে!' : 'Webhook URL কপি'}
                </button>
              </div>

              {/* Server Outbound IP for Binance Whitelist */}
              <div className="p-3 rounded-xl bg-[#090d18] border border-amber-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                <div>
                  <span className="text-[11px] font-bold text-amber-300 block flex items-center gap-1.5">
                    <span>🌐 হোস্টিং সার্ভার IP (Binance IP Restriction এর জন্য):</span>
                  </span>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="font-mono text-xs text-white font-black bg-[#0d1527] px-2 py-0.5 rounded-lg border border-amber-500/30 select-all">
                      34.96.48.13
                    </span>
                    <span className="text-[10px] text-slate-400">
                      (Binance API Management-এ এই IP টি Whitelist করুন অথবা 'Unrestricted' নির্বাচন করুন)
                    </span>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    navigator.clipboard.writeText('34.96.48.13');
                    setCopiedServerIp(true);
                    setTimeout(() => setCopiedServerIp(false), 2000);
                  }}
                  className="px-3 py-1.5 rounded-lg bg-amber-400/20 hover:bg-amber-400/30 text-amber-300 font-bold text-xs shrink-0 cursor-pointer transition-colors"
                >
                  {copiedServerIp ? '✓ কপি হয়েছে!' : 'Server IP কপি'}
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                <div>
                  <label className="block font-bold text-amber-300 mb-1">
                    Binance Pay API Key (Certificate SN) *
                  </label>
                  <input
                    type="text"
                    value={paymentSettings.binancePayApiKey || ''}
                    onChange={(e) => setPaymentSettings({ ...paymentSettings, binancePayApiKey: e.target.value })}
                    placeholder="আপনার Binance Merchant Certificate SN / API Key"
                    className="w-full bg-[#090e18] border border-amber-500/30 rounded-xl p-2.5 text-xs text-white focus:outline-none focus:border-amber-400 font-mono"
                  />
                </div>

                <div>
                  <label className="block font-bold text-amber-300 mb-1 flex items-center justify-between">
                    <span>Binance Pay Secret Key *</span>
                    <button
                      type="button"
                      onClick={() => setShowBinanceSecret(!showBinanceSecret)}
                      className="text-[10px] text-slate-400 hover:text-white cursor-pointer"
                    >
                      {showBinanceSecret ? 'গোপন করুন' : 'দেখুন'}
                    </button>
                  </label>
                  <input
                    type={showBinanceSecret ? 'text' : 'password'}
                    value={paymentSettings.binancePaySecretKey || ''}
                    onChange={(e) => setPaymentSettings({ ...paymentSettings, binancePaySecretKey: e.target.value })}
                    placeholder="আপনার Binance Merchant Secret Key"
                    className="w-full bg-[#090e18] border border-amber-500/30 rounded-xl p-2.5 text-xs text-white focus:outline-none focus:border-amber-400 font-mono"
                  />
                </div>
                ...
                <div className="sm:col-span-2">
                  <label className="block font-bold text-slate-300 mb-1">
                    Binance Pay Merchant ID (ঐচ্ছিক / Optional)
                  </label>
                  <input
                    type="text"
                    value={paymentSettings.binancePayMerchantId || ''}
                    onChange={(e) => setPaymentSettings({ ...paymentSettings, binancePayMerchantId: e.target.value })}
                    placeholder="e.g. 123456789 (প্রয়োজনীয় না হলে খালি রাখতে পারেন)"
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-1">
                <button
                  ...
                  disabled={binanceTestLoading || !paymentSettings.binancePayApiKey}
                  className="px-4 py-2 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 text-xs font-bold flex items-center gap-2 cursor-pointer transition-all disabled:opacity-40"
                >
                  {binanceTestLoading ? '⏳ সংযোগ যাচাই করা হচ্ছে...' : '⚡ Binance Pay API টেস্ট করুন (Test Connection)'}
                </button>
